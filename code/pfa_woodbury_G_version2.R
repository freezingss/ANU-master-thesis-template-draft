...
fit_pfa_woodbury_G_multistart <- function(Y, X, group, K, M = rowSums(Y),
  ...
  for (s in seq_len(n_starts)) {
    perturb_sd <- if (s == 1) 0 else init_perturb_sd   # start 1 = the plain deterministic init
    seed_s <- base_seed + s
    ...
    fit_s <- withCallingHandlers(
      fit_pfa_woodbury_G(Y, X, group, K, M = M,
                          ...
                          corr_max_rel = corr_max_rel,
                          estep_max_iter = estep_max_iter, estep_gtol = estep_gtol,
                          init_perturb_sd = perturb_sd, init_seed = seed_s),
      warning = function(w) {
    ...
    cat(sprintf("start %2d (perturb_sd=%.2f, seed=%d): iters=%3d  G_final=%.4f%s%s\n",
                s, perturb_sd, seed_s, iters[s], G_final[s],
  ...
  summary_df <- data.frame(start = seq_len(n_starts), iters = iters, G_final = G_final)
...
if (interactive() || !exists("SKIP_RUN_SCRIPT")) {
  ...
  cat(sprintf("G final (iter %d): Bd=%.4f\n\n",
  ...
  cat("\n sanity check: forced long run, tol effectively off\n")
  fit_G_forced <- fit_pfa_woodbury_G(dat$Y, dat$X, dat$group, K_true, M = dat$M,
                                      max_iter = 150, tol = 1e-14, sigma2_init = 0.3,
                                      verbose = FALSE, B_true = dat$true$B, fix_sigma2 = NULL)
  cat(sprintf("forced run: iters=%d (max_iter=150), G_final=%.6f, Bd_final=%.4f\n",
              fit_G_forced$iterations, tail(fit_G_forced$G, 1), tail(fit_G_forced$B_dist_trace, 1)))
  cat("if this ran to iters < 150 with no G-decrease warning above, that is a genuine\n")
  cat("stationary point, not a coincidental early stop.\n")

  cat("\n multi-start: unfrozen sigma2, 5 starts\n")
  ms <- fit_pfa_woodbury_G_multistart(dat$Y, dat$X, dat$group, K_true, M = dat$M,
                                       n_starts = 5, init_perturb_sd = 0.3, base_seed = 1,
                                       max_iter = 100, tol = 1e-10, sigma2_init = 0.3,
                                       verbose = FALSE, B_true = dat$true$B, fix_sigma2 = NULL)

  op <- par(mfrow = c(1, 2))
  ylim_bd <- range(c(fit_corr$B_dist_trace, fit_G$B_dist_trace, fit_G_frozen$B_dist_trace))
  xlim_it <- c(1, max(length(fit_corr$B_dist_trace), length(fit_G$B_dist_trace), length(fit_G_frozen$B_dist_trace)))
  plot(fit_corr$B_dist_trace, type = "l", col = "blue", lwd = 2, xlim = xlim_it, ylim = ylim_bd,
       xlab = "iteration", ylab = "B_dist",
       main = "B_dist: corr vs G vs G+frozen")
  lines(fit_G$B_dist_trace, col = "black", lwd = 2)
  lines(fit_G_frozen$B_dist_trace, col = "darkgreen", lwd = 2)
  legend("topright", c("old corr", "new G", "new G+frozen"), col = c("blue", "black", "darkgreen"), lty = 1, lwd = 2)

  plot(fit_G$G, type = "l", col = "black", lwd = 2, xlab = "iteration", ylab = "G",
       main = "G trace: must be non-decreasing -- this IS the falsification test",
       ylim = range(c(fit_G$G, fit_G_frozen$G)))
  lines(fit_G_frozen$G, col = "darkgreen", lwd = 2)
  legend("bottomright", c("new G", "new G+frozen"), col = c("black", "darkgreen"), lty = 1, lwd = 2)
  par(op)
}
