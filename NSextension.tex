...
\begin{lemma}[Loading-subspace Information Upper Bound]
    Under pervasive assumption $\lambda_{K}(\mathbf{B}^{*\top}\mathbf{B}^{*})\asymp Q$, for any unit direction out-of-subspace, the expected Fisher information $\bar{\mathbf{I}}_{\mathbf{BB}}(\dot{\mathbf{B}})=J\sigma^{-2}+\mathcal{o}(J)$, which is independent to the increase race of $Q$ and $I_{j}$. Therefore, the information of loading row space is bounded and $\|\sin \Theta(\hat{\mathbf{B}},\mathbf{B}^{*})\| \succsim cJ^{-\frac{1}{2}}$ with $J$ fixed and $Q\to\infty$.
\end{lemma}