...
fit_pfa_fh_em_pure <- function(Y, X, group, K,
                               M = rowSums(Y),
  ...
  estep_iso_fn <- function(mu, phi, tau2)
    estep_wbonly(J, group, Y, X, M, mu, phi, matrix(0, Q, K), tau2, estep_max_iter, estep_gtol)
  mstep_phi_fn <- function(lambda_hat, mu, phi)
    mstep_phi_wbonly(Y, X, group, lambda_hat, mu, phi, lambda_phi = lambda_phi)

  fit <- fit_fhem(K = K, estep_iso_fn = estep_iso_fn, mstep_phi_fn = mstep_phi_fn,
