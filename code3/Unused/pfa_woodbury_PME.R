# pfa_woodbury_PME.R
laplace_lambda_j_wbpme <- function(Y_j, X_j, M_j, mu, phi, B, sigma2,
...
estep_wbpme <- function(J, group, Y, X, M, mu, phi, B, sigma2,
  ...
  for (j in 1:J) {
    ...
    res <- laplace_lambda_j_wbpme(Y[idx, , drop = FALSE], X[idx, , drop = FALSE],
...
fit_pfa_wbpme_traced <- function(Y, X, group, K,
                                 M = rowSums(Y),
  ...
  for (em in 1:max_iter) {
    es <- estep_wbpme(J, group, Y, X, M, mu, phi, B, sigma2,
