# pfa_PME_only.R
laplace_lambda_j_pme <- function(Y_j, X_j, M_j, mu, phi, B, sigma2,
...
estep_pme <- function(J, group, Y, X, M, mu, phi, B, sigma2,
  ...
  for (j in 1:J) {
    ...
    res <- laplace_lambda_j_pme(Y[idx, , drop = FALSE], X[idx, , drop = FALSE],
...
fit_pfa_pme_traced <- function(Y, X, group, K,
                               M = rowSums(Y),
  ...
  for (em in 1:max_iter) {
    es <- estep_pme(J, group, Y, X, M, mu, phi, B, sigma2,
