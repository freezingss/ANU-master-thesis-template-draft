...
fit_fhem_woodbury <- function(Y, X, group, J, mu, phi, K, B_init, sigma2_init,
                              max_iter = 100, iso_gtol = 1e-8, sigma2_floor = 1e-8,
  ...
  for (it in 1:max_iter) {
    ...
    for (j in 1:J) {
      ...
      iso_list[[j]] <- iso_newton_group(Y_j, M_j, Eta0_j, tau2, gtol = iso_gtol)
    }
    ...
    if (delta_sigma2 < tol && delta_B < 1e-6) break
  }
  ...
  for (j in 1:J) {
    ...
    M_j <- rowSums(Y_j)
    Eta0_j <- matrix(mu, length(idx), Q, byrow = TRUE) + X[idx, , drop = FALSE] %*% phi
    iso_list_final[[j]] <- iso_newton_group(Y_j, M_j, Eta0_j, tau2_final, gtol = iso_gtol)
  }
