# Old tests with glmmTMB(): using standard multinomial, woodbury accelerated and woodbury accelerated with lambda correction. 
# 4 results compared.
# More work: add the runtime threshold for standard multinomial and glmmTMB() in 
# the large regime of J and Q, they are too slow!

setwd("F:/2026S1/Adv/research project/code/owwek/ANU-honours-thesis-template-draft/code3")
getwd()

source("pfa_base_multinomial.R") # base multinomial
source("pfa_woodbury_only.R") # woodbury
source("pfa_woodbury_corr.R") # woodbury + lambda correction
# source("pfa_") #ISO
source("pfa_debias.R") # debias
source("pfa_fh_em.R") # Fay-Herriot EM
source("sim_data.R")
source("basic_functions.R")

stopifnot(packageVersion("glmmTMB") >= "1.1.8") # rr() facor analysis required
...
for (J in Js) {
  for (Q in Qs) {
    for (seed in seeds) {
      ...
      b_wb <- bench::mark(
        fw_only <<- fit_pfa_wbonly(dat$Y, dat$X, dat$group, K = K, M = dat$M,
                                     max_iter = 80, tol = 1e-3, sigma2_init = target, 
                                     verbose = FALSE, estep_max_iter = 100,
      ...
      # debias
      b_debias <- bench::mark(
        fw_debias <<- fit_pfa_debias(dat$Y, dat$X, dat$group, K, M = dat$M,
                                     max_iter = 80, tol = 1e-3, sigma2_init = target,
                                     verbose = FALSE, estep_max_iter = 100, estep_gtol = 1e-3,
                                     B_true = NULL),
        iterations = 1, check = FALSE, memory = FALSE)
      t_debias <- as.numeric(b_debias$median)
      
      # FH-EM
      b_fh <- bench::mark(
        fw_fh <<- fit_pfa_fh_em(dat$Y, dat$X, dat$group, K, M = dat$M,
                                max_iter = 80, tol = 1e-3, sigma2_init = target,
                                verbose = FALSE, estep_max_iter = 100, estep_gtol = 1e-3,
                                B_true = NULL),
        iterations = 1, check = FALSE, memory = FALSE)
      t_fh <- as.numeric(b_fh$median)
      ...
      d_debias_true <- subspace_dist(fw_debias$B, dat$true$B)
      d_fh_true <- subspace_dist(fw_fh$B, dat$true$B)
      ...
      rows <- data.frame(
        J = J, Q = Q, seed = seed,
        method = c("std","woodbury only", "corrected", "glmmTMB rr()"),
