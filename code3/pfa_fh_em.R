ppca_closed <- function(S, K) {
  ...
  ev <- e$values
  q <- length(ev)
  sigma2 <- max(mean(ev[(K + 1):q]), 1e-8)
  B <- e$vectors[, 1:K, drop = FALSE] %*% diag(sqrt(pmax(ev[1:K] - sigma2, 0)), K)
  list(B = B, sigma2 = sigma2, Sigma = tcrossprod(B) + sigma2 * diag(q))
}
...
fhem_backsolve <- function(lambda_hat, S_hat, tau2, ridge = 1e-3) {
  ...
  for (j in 1:J) {
    ...
    lt[[j]] <- solve(Fj_reg, Sh_inv %*% lambda_hat[, j])
    Fl[[j]] <- Fj_reg
    Fl_inv[[j]] <- solve(Fj_reg)
  }
  list(lt = lt, Fl = Fl, Fl_inv = Fl_inv)
}
...
fit_fhem <- function(K, estep_iso_fn, mstep_phi_fn, apply_PLT_fn = identity,
  ...
  trace <- data.frame(iter = integer(0), tau2 = numeric(0), sigma2 = numeric(0), fh_ll = numeric(0))
  fh <- NULL
  for (outer in 1:max_iter) {
    ...
    ll <- tail(fh$trace, 1)
    trace <- rbind(trace, data.frame(iter = outer, tau2 = tau2, sigma2 = sigma2, fh_ll = ll))
